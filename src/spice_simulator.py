...
import logging # Added for direct use if app_logger is not sufficient
...
def perform_dc_spice_analysis(current_netlist_content, app_logger):
    ...
    """
    if not current_netlist_content or not current_netlist_content.strip():
        error_msg = "Netlist is empty. Cannot perform DC SPICE analysis."
    ...
    try:
        ...
        app_logger.debug("Full DC SPICE netlist with control commands (for debugging):")
        ...
        # Detailed logging of raw results can be very verbose, consider if needed for all log levels
        # For now, keeping it at DEBUG level
        if app_logger.isEnabledFor(logging.DEBUG):
            for node, value in analysis.nodes.items():
                app_logger.debug(f"  Node {node}: {value} (type: {type(value)})")
            for branch, value in analysis.branches.items():
                app_logger.debug(f"  Branch {branch}: {value} (type: {type(value)})")
        ...
        for branch, value in analysis.branches.items():
            try:
                ...
                # Standard engineering practice is often mA for branch currents in small circuits
        ...
        return {
            "status": "success",
            "dc_netlist": net_text_dc, # For debugging or display
            "node_voltages": node_voltages_dc,
            "branch_currents": branch_currents_dc
        }

    except Exception as e_dc:
        error_msg_dc = f"DC SPICE Analysis Error: {str(e_dc)}"
        app_logger.error(error_msg_dc, exc_info=True)
        return {"status": "error", "message": error_msg_dc}

def perform_ac_spice_analysis(active_results_data_dict, analyzer_instance, current_ac_frequency_hz, app_logger):
    """Performs AC SPICE analysis, including parsing, simulation, and plotting.
    Returns a dictionary with results, including a Matplotlib figure object, or error information.
    """
    fig = None # Initialize fig to None
    try:
        if not active_results_data_dict or not active_results_data_dict.get('netlist'):
            error_msg = "Netlist not available in active_results_data_dict for AC analysis."
            app_logger.error(error_msg)
            return {"status": "error", "message": error_msg, "phasor_plot_fig": None}

        sim_netlist_data = deepcopy(active_results_data_dict['netlist'])
        spice_body_lines = []
        ...
        for line_dict in sim_netlist_data:
            ...
            if component_type_prefix in ['V', 'I']:
                ...
                if parsed_params:
                    ...
                    app_logger.debug(f"Processed AC source {component_name_debug}: original='{original_value}', spice_val='{line_dict['value']}'")
                else:
                    if original_value.lower().strip().startswith('ac') or ':' in original_value:
                        default_ac_val = "0 AC 1 0" # Default: 0V DC offset, 1V AC magnitude, 0 phase
                        app_logger.warning(f"Could not parse AC parameters for {component_name_debug}: '{original_value}'. Using default: {default_ac_val}")
                        line_dict['value'] = default_ac_val
                    # Else, if not clearly an AC source string, assume it's DC and leave as is for PySpice to handle.
            elif component_type_prefix == 'C':
                val_lower = original_value.lower()
                if val_lower.startswith("-j"): # Reactance form -jXc
                    try:
                        reactance_str = val_lower[2:]
                        Xc = float(reactance_str) if reactance_str else 1.0 # Avoid empty string for float conversion
                        if Xc <= 0 or current_ac_frequency_hz <= 0:
                            app_logger.warning(f"Invalid Xc ({Xc}) or freq ({current_ac_frequency_hz}Hz) for {component_name_debug}. Using original value: '{original_value}'")
                        else:
                            capacitance = 1 / (2 * np.pi * current_ac_frequency_hz * Xc)
                            line_dict['value'] = capacitance # Store capacitance in Farads
                        ...
                        app_logger.warning(f"Could not parse Xc from '{original_value}' for {component_name_debug}. Using original value.")
            elif component_type_prefix == 'L':
                val_lower = original_value.lower()
                Xl, parsed_Xl = None, False
                if val_lower.startswith("j"): # Reactance form jXl (no minus)
                    try:
                        Xl = float(val_lower[1:]) if val_lower[1:] else 1.0
                        parsed_Xl = True
                    except ValueError:
                        app_logger.warning(f"Could not parse Xl from '{original_value}' (jXl format) for {component_name_debug}. Using original value.")
                # Some conventions might use Xlj instead of jXl
                elif 'j' in val_lower and val_lower.endswith('j') and not val_lower.startswith('-'):
                    try:
                        Xl = float(val_lower[:-1]) if val_lower[:-1] else 1.0
                        parsed_Xl = True
                    except ValueError:
                        app_logger.warning(f"Could not parse Xl from '{original_value}' (Xlj format) for {component_name_debug}. Using original value.")

                if parsed_Xl and Xl is not None:
                    if Xl <= 0 or current_ac_frequency_hz <= 0:
                        app_logger.warning(f"Invalid Xl ({Xl}) or freq ({current_ac_frequency_hz}Hz) for {component_name_debug}. Using original value: '{original_value}'")
                    else:
                        inductance = Xl / (2 * np.pi * current_ac_frequency_hz)
                        line_dict['value'] = inductance # Store inductance in Henrys
        ...
        if not netlist_content_for_pyspice.strip():
            error_msg = "Netlist for AC analysis is effectively empty after processing. Cannot simulate."
            app_logger.error(error_msg)
            return {"status": "error", "message": error_msg, "phasor_plot_fig": None}
        ...
        app_logger.debug("Full AC SPICE netlist (for debugging):")
        ...
        for node_name_ac, val_waveform_ac in analysis_ac.nodes.items():
            ...
            if not is_complex and hasattr(val_waveform_ac, '__len__') and len(val_waveform_ac) > 0: # Fallback for non-complex arrays
        ...
        for branch_name, val_waveform in analysis_ac.branches.items():
            ...
            if not is_complex_b and hasattr(val_waveform, '__len__') and len(val_waveform) > 0: # Fallback
            ...
            if value_to_convert_b is not None:
                ...
                # Currents are typically in mA or A depending on magnitude
                unit = "A"
                if abs(mag) < 1.0 and abs(mag) > 1e-6 : # If less than 1A but not microamps, use mA
                    mag *= 1000
                    unit = "mA"
                branch_currents_ac_display[str(branch_name)] = f"{mag:.3f} ∠ {phase:.2f}° {unit}"
            else:
        ...
        try:
            ...
            for node, val_waveform in analysis_ac.nodes.items():
                if hasattr(val_waveform, '__len__') and len(val_waveform) > 0 and str(node) != '0': # Ensure waveform has data
            ...
            for branch, val_waveform in analysis_ac.branches.items():
                 if hasattr(val_waveform, '__len__') and len(val_waveform) > 0: # Ensure waveform has data
                    complex_val = safe_to_complex(val_waveform[0])
                    mag, angle = np.abs(complex_val), np.angle(complex_val)
            ...
            plt.tight_layout()
            # Do NOT call plt.close(fig) here if returning the fig object
        except Exception as e_plot:
            app_logger.error(f"Error generating phasor plots: {str(e_plot)}")
            if fig: plt.close(fig) # Close if error occurred during plot creation
            fig = None # Ensure fig is None if plotting failed

        return {
            "status": "success",
            "ac_netlist": net_text_ac, # For debugging or display
            "node_voltages": node_voltages_ac_display,
            "branch_currents": branch_currents_ac_display,
            "phasor_plot_fig": fig # Can be None if plotting failed
        }

    except Exception as e_ac:
        error_msg_ac = f"AC SPICE Analysis Error: {str(e_ac)}"
        app_logger.error(error_msg_ac, exc_info=True)
        if fig: # If a figure was created before this broader exception
            plt.close(fig)
        return {"status": "error", "message": error_msg_ac, "phasor_plot_fig": None}
